...
@section('header-subtitle', 'Perbarui konfigurasi menu navigasi sidebar')
...
<div class="max-w-2xl space-y-6">
    ...
    <div class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-[0_5px_25px_rgba(8,21,66,0.03)]">
        ...
        <div class="flex items-center justify-between pb-5 border-b border-slate-100 mb-6">
            <div>
                <h2 class="text-base font-bold text-slate-900 font-heading">Edit Menu: {{ $menu->title }}</h2>
                <p class="text-xs text-slate-400">ID Menu: #{{ $menu->id }} • Status: {{ $menu->is_active ? 'Aktif' : 'Nonaktif' }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.menus.update', $menu) }}" class="space-y-5" novalidate>
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input 
                    ...
                    class="mt-1.5 block w-full font-mono rounded-2xl border border-slate-200 focus:border-[#24695c] focus:ring-[#24695c] px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 bg-slate-50/50 focus:bg-white focus:outline-none focus:ring-2 transition-all"
                >
                <p class="mt-1 text-[11px] text-slate-400">Contoh: <code>heroicon-o-squares-2x2</code>, <code>heroicon-o-users</code>, <code>heroicon-o-shield-check</code>.</p>
            </div>
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($permissions as $perm)
                            <option value="{{ $perm->name }}" {{ old('permission_name', $menu->permission_name) == $perm->name ? 'selected' : '' }}>
                                {{ $perm->name }}
                            </option>
                        @endforeach
                    </select>
