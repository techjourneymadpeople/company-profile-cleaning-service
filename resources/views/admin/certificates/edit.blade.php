...
@section('title', 'Edit Sertifikat: ' . $certificate->name)
@section('header-title', 'Edit Certificate')
@section('header-subtitle', 'Perbarui data sertifikasi atau perpanjang masa berlaku')
...
<div class="max-w-2xl space-y-6">
    ...
    <div class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-[0_5px_25px_rgba(8,21,66,0.03)]">
        ...
        <div class="flex items-center justify-between pb-5 border-b border-slate-100 mb-6">
            <div>
                <h2 class="text-base font-bold text-slate-900 font-heading">Edit Sertifikat: {{ $certificate->name }}</h2>
                <p class="text-xs text-slate-400">ID: #{{ $certificate->id }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.certificates.update', $certificate) }}" enctype="multipart/form-data" class="space-y-5" novalidate>
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    @if($certificate->image_path)
                        <div class="mb-2 p-2 bg-slate-50 border border-slate-200 rounded-xl inline-block">
                            <img src="{{ asset('storage/' . $certificate->image_path) }}" alt="Preview Sertifikat" class="h-12 w-auto object-contain">
                        </div>
                    @endif
                    <input 
                        id="image" 
