...
@section('header-subtitle', 'Perbarui data mitra dan file logo')
...
<div class="max-w-xl space-y-6">
    ...
    <div class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-[0_5px_25px_rgba(8,21,66,0.03)]">
        ...
        <div class="flex items-center justify-between pb-5 border-b border-slate-100 mb-6">
            <div>
                <h2 class="text-base font-bold text-slate-900 font-heading">Edit Klien: {{ $client->name }}</h2>
                <p class="text-xs text-slate-400">ID: #{{ $client->id }}</p>
            </div>
            <a href="{{ route('admin.clients.index') }}" class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-2xl text-xs font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 border border-slate-200 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#24695c]">
                <x-heroicon-o-arrow-left class="w-4 h-4" aria-hidden="true" />
                <span>Kembali</span>
            </a>
        </div>

        <form method="POST" action="{{ route('admin.clients.update', $client) }}" enctype="multipart/form-data" class="space-y-5" novalidate>
            @csrf
            @method('PUT')
            ...
            <div>
                <label for="logo" class="block text-xs font-bold uppercase tracking-wider text-slate-700 font-heading mb-1.5">
                    File Logo Klien (PNG Transparan / SVG)
                </label>
                @if($client->logo_path)
                    <div class="mb-2 p-2 bg-slate-50 border border-slate-200 rounded-xl inline-block">
                        <img src="{{ asset('storage/' . $client->logo_path) }}" alt="Logo Saat Ini" class="h-10 max-w-[150px] object-contain">
                    </div>
                @endif
                <input 
                    id="logo" 
            ...
            <div>
                <label for="sort_order" class="block text-xs font-bold uppercase tracking-wider text-slate-700 font-heading">
                    Urutan Tampil (Sort Order) <span class="text-rose-500">*</span>
                </label>
                <input 
                    id="sort_order" 
                    name="sort_order" 
            ...
            <div class="pt-2">
                <label class="inline-flex items-center cursor-pointer">
                    <input 
                        ...
                        name="is_visible" 
                        value="1" 
                        {{ old('is_visible', $client->is_visible) ? 'checked' : '' }}
                        class="h-4 w-4 rounded border-slate-300 text-[#24695c] focus:ring-[#24695c]"
                    >
                    <span class="ml-2.5 text-xs font-bold text-slate-800 select-none">Tampilkan Logo di Halaman Depan (Show)</span>
                </label>
            </div>

            <!-- Action Buttons -->
            <div class="pt-5 border-t border-slate-100 flex items-center justify-end gap-3">
                <a href="{{ route('admin.clients.index') }}" class="px-5 py-3 rounded-2xl text-xs font-bold text-slate-600 hover:bg-slate-100 transition-colors font-heading uppercase">
                    Batal
                </a>
