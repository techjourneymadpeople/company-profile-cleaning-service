...
@section('header-title', 'Edit Article')
@section('header-subtitle', 'Perbarui konten artikel, kategori, dan status publikasi')
...
<div class="max-w-4xl space-y-6">
    ...
    <div class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-[0_5px_25px_rgba(8,21,66,0.03)]">
        ...
        <div class="flex items-center justify-between pb-5 border-b border-slate-100 mb-6">
            <div>
                <h2 class="text-base font-bold text-slate-900 font-heading">Edit Artikel: {{ Str::limit($article->title, 40) }}</h2>
                <p class="text-xs text-slate-400">ID: #{{ $article->id }} • Penulis: {{ $article->author->name ?? 'Admin' }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.articles.update', $article) }}" enctype="multipart/form-data" class="space-y-6" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    >
                        <option value="Edukasi Kebersihan" {{ old('category', $article->category) === 'Edukasi Kebersihan' ? 'selected' : '' }}>Edukasi Kebersihan</option>
                        <option value="Berita Perusahaan" {{ old('category', $article->category) === 'Berita Perusahaan' ? 'selected' : '' }}>Berita Perusahaan</option>
                        <option value="Tips & Trik Fasilitas" {{ old('category', $article->category) === 'Tips & Trik Fasilitas' ? 'selected' : '' }}>Tips & Trik Fasilitas</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="draft" {{ old('status', $article->status) === 'draft' ? 'selected' : '' }}>Draft (Simpan Konsep)</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                @if($article->featured_image)
                    <div class="mb-2 p-2 bg-slate-100 rounded-2xl inline-block border border-slate-200">
                        <img src="{{ asset('storage/' . $article->featured_image) }}" alt="Featured Image" class="h-16 w-auto rounded-xl object-cover">
                    </div>
                @endif
                <input 
                    id="featured_image" 
