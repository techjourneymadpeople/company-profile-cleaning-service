...
@section('title', 'Edit Angka Pencapaian: ' . $statistic->label)
@section('header-title', 'Edit Counter')
@section('header-subtitle', 'Perbarui nilai statistik dan urutan tampil')
...
<div class="max-w-xl space-y-6">
    ...
    <div class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-[0_5px_25px_rgba(8,21,66,0.03)]">
        ...
        <div class="flex items-center justify-between pb-5 border-b border-slate-100 mb-6">
            <div>
                <h2 class="text-base font-bold text-slate-900 font-heading">Edit Counter: {{ $statistic->label }}</h2>
                <p class="text-xs text-slate-400">ID: #{{ $statistic->id }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.statistics.update', $statistic) }}" class="space-y-5" novalidate>
            @csrf
            @method('PUT')
