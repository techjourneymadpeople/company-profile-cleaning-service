...
@section('title', 'Tambah Testimoni Klien Baru')
@section('header-title', 'Create Testimonial')
@section('header-subtitle', 'Tambahkan ulasan kepuasan dari klien atau mitra perusahaan')
...
<div class="max-w-2xl space-y-6">
    ...
    <div class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-[0_5px_25px_rgba(8,21,66,0.03)]">
        ...
        <div class="flex items-center justify-between pb-5 border-b border-slate-100 mb-6">
            <div>
                <h2 class="text-base font-bold text-slate-900 font-heading">Formulir Tambah Testimoni</h2>
                <p class="text-xs text-slate-400">Lengkapi kutipan kepuasan dan rating bintang</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.testimonials.store') }}" enctype="multipart/form-data" class="space-y-5" novalidate>
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input 
                        ...
                        type="text" 
                        value="{{ old('designation_company') }}"
                        placeholder="Contoh: Facility Manager - PT XYZ"
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="4" {{ old('rating') == 4 ? 'selected' : '' }}>⭐⭐⭐⭐ (4 Bintang - Sangat Puas)</option>
                        <option value="3" {{ old('rating') == 3 ? 'selected' : '' }}>⭐⭐⭐ (3 Bintang - Cukup Puas)</option>
                        <option value="2" {{ old('rating') == 2 ? 'selected' : '' }}>⭐⭐ (2 Bintang - Kurang Puas)</option>
                        <option value="1" {{ old('rating') == 1 ? 'selected' : '' }}>⭐ (1 Bintang - Tidak Puas)</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea 
                    ...
                    rows="4" 
                    required 
                    placeholder="Tulis ulasan kepuasan klien mengenai kualitas layanan kebersihan..."
                    class="mt-1.5 block w-full rounded-2xl border border-slate-200 focus:border-[#24695c] focus:ring-[#24695c] px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 bg-slate-50/50 focus:bg-white focus:outline-none focus:ring-2 transition-all"
                >{{ old('quote') }}</textarea>
