...
@section('header-subtitle', 'Perbarui data akun pengguna dan kelola penugasan role')
...
<div class="max-w-3xl space-y-6">
    ...
    <div class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-[0_5px_25px_rgba(8,21,66,0.03)]">
        ...
        <div class="flex items-center justify-between pb-5 border-b border-slate-100 mb-6">
            <div>
                <h2 class="text-base font-bold text-slate-900 font-heading">Edit Akun: {{ $user->name }}</h2>
                <p class="text-xs text-slate-400">ID Pengguna: #{{ $user->id }} • Terdaftar: {{ $user->created_at->format('d M Y') }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-6" novalidate>
            ...
            <div class="p-5 rounded-2xl bg-slate-50/80 border border-slate-200/60 space-y-4">
                <div>
                    <h3 class="text-xs font-bold uppercase tracking-wider text-slate-700 font-heading">Ganti Kata Sandi (Opsional)</h3>
                    <p class="text-[11px] text-slate-400">Kosongkan kolom berikut jika tidak ingin mengubah kata sandi saat ini.</p>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label for="password" class="block text-xs font-medium text-slate-700">Kata Sandi Baru</label>
                        <div class="mt-1 relative rounded-2xl">
                            <input 
                                ...
                                type="password" 
                                placeholder="••••••••"
                                class="block w-full rounded-2xl border {{ $errors->has('password') ? 'border-rose-400 focus:border-rose-600' : 'border-slate-200 focus:border-[#24695c] focus:ring-[#24695c]' }} pl-3.5 pr-10 py-2.5 text-sm text-slate-900 bg-white focus:outline-none focus:ring-2 transition-all"
                            >
                            <button 
                                type="button" 
                                onclick="togglePasswordVisibility('password', this)" 
                                class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-[#24695c] focus:outline-none cursor-pointer transition-colors" 
                                aria-label="Tampilkan kata sandi baru"
                            >
                                <x-heroicon-o-eye class="w-4 h-4 eye-icon" aria-hidden="true" />
                                <x-heroicon-o-eye-slash class="w-4 h-4 eye-slash-icon hidden" aria-hidden="true" />
                            </button>
                        </div>
                        @error('password')
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-xs font-medium text-slate-700">Ulangi Kata Sandi Baru</label>
                        <div class="mt-1 relative rounded-2xl">
                            <input 
                                ...
                                type="password" 
                                placeholder="••••••••"
                                class="block w-full rounded-2xl border border-slate-200 focus:border-[#24695c] focus:ring-[#24695c] pl-3.5 pr-10 py-2.5 text-sm text-slate-900 bg-white focus:outline-none focus:ring-2 transition-all"
                            >
                            <button 
                                type="button" 
            ...
            <div class="pt-4 border-t border-slate-100">
                ...
                <p class="text-xs text-slate-500 mb-4">Perbarui penugasan role hak akses untuk akun ini:</p>

                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                    @foreach($roles as $role)
                        <label class="relative flex items-center p-4 rounded-2xl border {{ in_array($role->name, old('roles', $userRoleNames)) ? 'border-[#24695c] bg-[#e2f4f1]/30' : 'border-slate-200 bg-slate-50/50' }} hover:border-[#24695c] cursor-pointer transition-all">
                            <input 
                                type="checkbox" 
                                name="roles[]" 
                                value="{{ $role->name }}" 
